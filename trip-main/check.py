...
def open_region_window(region, master):
    ...
    def on_show_ranking():
        ...
        rank_dict = get_ranking_udp()
        if not rank_dict:
            tk.Label(weather_scroll_frame, text="❌ 순위 불러오기 실패", fg="red").pack()
            return
        ...
        def rank(sp): return -rank_dict.get(sp, 0)  # 조회수 높은 순

        indoor_sorted  = sorted(indoor_spots, key=rank)
        outdoor_sorted = sorted(outdoor_spots, key=rank)

        rank_frame = tk.Frame(weather_scroll_frame)
        rank_frame.pack(fill="both", padx=5)

        indoor_frame = tk.Frame(rank_frame)
        indoor_frame.pack(side="left", fill="y", padx=10)

        outdoor_frame = tk.Frame(rank_frame)
        outdoor_frame.pack(side="left", fill="y", padx=10)

        tk.Label(indoor_frame, text="🏠 실내 여행지 순위", font=("맑은 고딕", 10, "bold")).pack(anchor="w")
        for i, sp in enumerate(indoor_sorted, 1):
            tk.Label(indoor_frame, text=f"{i}. {sp} ({rank_dict.get(sp, 0)}회)",
                     font=("맑은 고딕", 10)).pack(anchor="w", padx=5)

        tk.Label(outdoor_frame, text="🌳 야외 여행지 순위", font=("맑은 고딕", 10, "bold")).pack(anchor="w")
        for i, sp in enumerate(outdoor_sorted, 1):
            tk.Label(outdoor_frame, text=f"{i}. {sp} ({rank_dict.get(sp, 0)}회)",
                     font=("맑은 고딕", 10)).pack(anchor="w", padx=5)
    ...
    # ✅ 버튼 내부 묶음: 중앙 정렬을 위한 컨테이너 프레임
    button_inner = tk.Frame(button_row)
    ...
    weather_btn.pack(side="left", padx=(0, 10))
    ...
    def toggle_accommodations():
        ...
        if not accom_state["show"]:
            ...
            for info in hotels:
                ...
                hb.config(command=lambda inf=info, b_ref=hb: show_accom_photo(inf, b_ref))
